...
def update_expenses_sql(date, amount, description, account, category, id_expenses):
    ...
    """
    datos = (date, description, amount, category, account, id_expenses)
    cursor.execute(consulta, datos)
    conexion.commit()
    cursor.close()
    conexion.close()
...
def view_expenses_sql(id_expenses):
    conexion = sqlite3.connect('presupuesto.db')
    cursor = conexion.cursor()
    consulta = """
    ...
    """
    cursor.execute(consulta, (id_expenses,))
...
def edit_expenses(request, id_expenses):
    transaction_data =view_expenses_sql(id_expenses)
    ...
    if request.POST:
        ...
        update_expenses_sql(date, amount, description, account, category, id_expenses)
